...
const Courses = () => {
  ...
  const state = useSelector((state) => state.courses.allCourses);
  ...
  if (!isLoading && !isError && courses?.courses?.length > 0) {
    // content = <CourseArea courseData={courses?.courses} />;
    content = <CourseArea courseData={state?.courses    } />;
  }
  ...
  return (
    <>
      ...
      {content}
      {/* {courses?.length > 0 ? <CourseArea courseData={courses} /> : ""} */}
