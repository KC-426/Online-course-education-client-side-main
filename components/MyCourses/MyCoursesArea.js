...
const MyCoursesArea = () => {
  ...
  const [course__, setCourse] = useState([]);
  ...
  const allCourse = async () => {
    try {
      const url = `${base_url}/api/v1/my_course`;
  ...
  console.log(course__);
  return (
    <>
      <section className="my__course pt-120 pb-90">
        <div className="container">
          <div className="row">
            {course__?.map((course_id) => {
                ...
                >
                  <div className="course__item-2 transition-3 white-bg mb-30 fix">
                    <div className="course__thumb-2 w-img fix course_thumb_height">
                      <Link href={`/enroll_course/${course?._id}`}>
                        <a>
                          <img src={course?.thumb_img?.url} alt="" />
                        </a>
                      </Link>
                    </div>
                    <div className="course__content-2">
                      <div className="course__top-2 d-flex align-items-center justify-content-between">
                        <div className="course__tag-2 d-flex gap-2">
                          {course?.category?.map((ele, i) => {
                            return <Link key={i} href={`/enroll_course/${course?._id}`}>
                              <a>{ele}</a>
                            </Link>;
                          }).slice(0, 2)}
                        </div>
